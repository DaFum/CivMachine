...
export function drawSettlementContent(surface: DrawSurface, scene: WorldScene, height: number, view: WorldBand): void {
  ...
  const bankCrest = (index: number): number => 4 + ridgeNoise(index * .5, civ.seed * 5 + 13, .55) * 26;
  const bankPeak = (index: number): number => index * 96 + 24 + hash01(civ.seed * 3 + index * 29) * 48;
