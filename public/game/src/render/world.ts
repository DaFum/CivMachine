...
interface BuildingShape { x: number; width: number; height: number; kind: number; }
...
function buildingLayout(civ: Civilization, worldWidth: number, height: number, count: number, stage: number): BuildingShape[] {
...
function drawSky(layer: any, civ: Civilization, worldWidth: number, height: number, presentation: ReturnType<typeof worldPresentation>, particleCount: number): void {
  layer.clear();
...
function drawTerrain(layer: any, civ: Civilization, worldWidth: number, height: number, presentation: ReturnType<typeof worldPresentation>): void {
  layer.clear();
...
function drawSettlement(layer: any, civ: Civilization, worldWidth: number, height: number, buildings: BuildingShape[], presentation: ReturnType<typeof worldPresentation>): void {
  layer.clear();
  ...
  if (stage > 0) {
    ...
    if (stage >= 2) layer.lineStyle(2, presentation.accent, .24).lineBetween(0, ground - 9, worldWidth, ground - 9);
    if (stage >= 4) layer.lineStyle(2, presentation.accent, .4).lineBetween(0, ground - 18, worldWidth, ground - 18);
  }
  ...
  for (const building of buildings) {
    ...
    if (stage === 0) {
      ...
      if (building.kind % 2 === 0) layer.fillStyle(0x56422c, .95).fillRect(left + building.width * .58, ground - building.height * .36, building.width * .28, building.height * .36);
...
function drawPathMotif(layer: any, civ: Civilization, worldWidth: number, height: number, ground: number, time: number, accent: number): void {
  ...
  switch (path) {
    ...
    case 'collective_mind': {
      ...
      for (let i = 1; i < points.length; i++) layer.lineBetween(points[i - 1].x, points[i - 1].y, points[i].x, points[i].y);
      for (const point of points) layer.fillStyle(accent, .5).fillCircle(point.x, point.y, 3);
      break;
    }
    ...
    case 'biological_transcendence':
      for (let i = 0; i < 18; i++) layer.fillStyle(accent, .14).fillCircle(worldWidth * hash01(civ.seed + i * 13), ground - 10 - hash01(i * 29) * 80, 8 + hash01(i) * 14);
      break;
    ...
    case 'recursive_simulation':
      for (let i = 0; i < 8; i++) {
        ...
        for (let ring = 0; ring < 3; ring++) layer.lineStyle(1, accent, .18 + .06 * ring).strokeRect(x - ring * 5, y - ring * 5, 22 + ring * 10, 14 + ring * 10);
      }
      break;
  }
}

function drawAtmosphere(layer: any, civ: Civilization, width: number, height: number, time: number, snapshot: ReturnType<typeof worldSnapshot>, presentation: ReturnType<typeof worldPresentation>, buildings: BuildingShape[]): void {
  layer.clear();
  ...
  for (let i = 0; i < snapshot.aircraftCount; i++) {
    ...
    const x = progress * worldWidth; const y = height * (.18 + hash01(i * 23) * .22);
...
function drawDecisionImpulse(layer: any, feedback: DecisionFeedback | null, startTime: number, time: number, width: number, height: number): void {
  layer.clear();
  ...
  if (reducedMotion) {
    ...
    if(kind==='time-streak')for(let i=0;i<4;i++)layer.lineStyle(2,color,.42).lineBetween(width*.18,height*(.38+i*.1),width*.82,height*(.38+i*.1));
    else if(kind==='scan'){layer.lineStyle(2,color,.48).lineBetween(width*.16,height*.5,width*.84,height*.5);layer.lineStyle(1,color,.4).strokeCircle(width*.5,height*.5,radius);}
    else if(kind==='fracture')for(let i=0;i<6;i++)layer.lineStyle(2,color,.44).lineBetween(width*(.3+i*.07),height*.3,width*(.34+i*.06),height*.72);
    else {layer.lineStyle(kind==='containment'?4:2,color,.48).strokeCircle(width*.5,height*.54,radius);layer.fillStyle(color,.06).fillCircle(width*.5,height*.54,radius*.72);}
    return;
  }
  ...
  if(kind==='containment'){
    for(let ring=0;ring<3;ring++)layer.lineStyle(4-ring, color, alpha*(1-ring*.18)).strokeCircle(width*.5,height*.54,radius*(.58+ring*.2));
    layer.fillStyle(color,alpha*.08).fillCircle(width*.5,height*.54,radius*.5);
  }else if(kind==='time-streak'){
    for(let i=0;i<9;i++){const y=height*(.2+i*.075);const inset=((i%3)*36+progress*width*.18)%Math.max(1,width*.28);layer.lineStyle(1.2+(i%2),color,alpha*(.45+(i%3)*.15)).lineBetween(-width*.08+inset,y,width*(.7+progress*.35)+inset,y);}
  }else if(kind==='scan'){
    const y=height*(.16+progress*.68);layer.lineStyle(2,color,alpha).lineBetween(width*.12,y,width*.88,y);layer.lineStyle(1,color,alpha*.75).strokeCircle(width*.5,height*.52,radius*.48);layer.lineBetween(width*.5-radius*.62,height*.52,width*.5+radius*.62,height*.52);layer.lineBetween(width*.5,height*.52-radius*.62,width*.5,height*.52+radius*.62);
  }else if(kind==='fracture'){
    for(let i=0;i<10;i++){const x=width*(.16+i*.075);const bend=(hash01(i*31+feedback.sequence)-.5)*width*.08;layer.lineStyle(1.2+(i%3),color,alpha).lineBetween(x,height*.18,x+bend,height*(.42+progress*.2));layer.lineBetween(x+bend,height*(.42+progress*.2),x-bend*.35,height*.84);}
  }else{
    layer.lineStyle(3-progress*2,color,alpha).strokeCircle(width*.5,height*.54,radius);layer.lineStyle(1,0xffffff,alpha*.5).strokeCircle(width*.5,height*.54,radius*.72);layer.fillStyle(color,alpha*.07).fillCircle(width*.5,height*.54,radius*.45);
  }
}

function drawCanvasDecisionImpulse(context:CanvasRenderingContext2D,feedback:DecisionFeedback,startTime:number,time:number,width:number,height:number,toColor:(value:number,alpha?:number)=>string):void{
  const age=time-startTime;if(age<0)return;
  const duration=reducedMotion?1400:1800;if(age>=duration)return;
  const kind=decisionImpulseKind(feedback.eventId);const color=impulseColor(feedback,kind);const progress=reducedMotion?0:Math.min(1,age/1800);const alpha=reducedMotion ? .48 : (1-progress)*.62;const radius=(reducedMotion ? .2 : .12+progress*.48)*Math.min(width,height);
  context.save();context.strokeStyle=toColor(color,alpha);context.fillStyle=toColor(color,alpha*.08);context.lineWidth=reducedMotion?2:Math.max(1,3-progress*2);
  if(kind==='time-streak'){for(let i=0;i<8;i++){const y=height*(.2+i*.08);context.beginPath();context.moveTo(width*(reducedMotion ? .18 : -.08+progress*.18),y);context.lineTo(width*(reducedMotion ? .82 : .72+progress*.3),y);context.stroke();}}
  else if(kind==='scan'){const y=reducedMotion?height*.5:height*(.16+progress*.68);context.beginPath();context.moveTo(width*.12,y);context.lineTo(width*.88,y);context.stroke();context.beginPath();context.arc(width*.5,height*.52,radius*.55,0,Math.PI*2);context.stroke();}
  else if(kind==='fracture'){for(let i=0;i<9;i++){const x=width*(.16+i*.08);const bend=(hash01(i*31+feedback.sequence)-.5)*width*.08;context.beginPath();context.moveTo(x,height*.18);context.lineTo(x+bend,height*.5);context.lineTo(x-bend*.35,height*.84);context.stroke();}}
  else{const rings=kind==='containment'?3:1;for(let ring=0;ring<rings;ring++){context.beginPath();context.arc(width*.5,height*.54,radius*(.68+ring*.2),0,Math.PI*2);context.stroke();}context.beginPath();context.arc(width*.5,height*.54,radius*.45,0,Math.PI*2);context.fill();}
  context.restore();
}

class FallbackWorld implements WorldController {
  ...
  private snapshot: ReturnType<typeof worldSnapshot> | null = null;
  private buildings: BuildingShape[] = [];
  ...
  private drawStatic(civ: Civilization, presentation: ReturnType<typeof worldPresentation>): void {
    ...
    for (let i = 0; i < snapshot.particleCount; i++) {
      ...
      context.beginPath(); context.arc(hash01(civ.seed + i * 17) * worldWidth, hash01(civ.seed + i * 31) * this.height * .58, .6 + hash01(i * 7) * 1.6, 0, Math.PI * 2); context.fill();
    }
    ...
    for (let i = 0; i < Math.ceil(worldWidth / 160) + 1; i++) {
      const x = i * 160 - 80; const peak = ground - 60 - hash01(civ.seed * 3 + i * 29) * 100;
      context.beginPath(); context.moveTo(x, ground); context.lineTo(x + 110, peak); context.lineTo(x + 230, ground); context.fill();
    }

    context.setTransform(devicePixelRatio, 0, 0, devicePixelRatio, -this.scroll * devicePixelRatio, 0);
    context.fillStyle = this.color(presentation.colors.nearTerrain); context.fillRect(0, ground, worldWidth, this.height - ground);
    context.fillStyle = snapshot.stage === 0 ? '#493821' : '#11191f'; context.fillRect(0, ground + 4, worldWidth, snapshot.stage === 0 ? 11 : 23);
    for (const building of this.buildings) {
      const left = building.x - building.width / 2; const top = ground - building.height;
      if (snapshot.stage === 0) {
        context.fillStyle = '#765c39'; context.beginPath(); context.moveTo(left, ground); context.lineTo(building.x, top); context.lineTo(left + building.width, ground); context.fill();
        context.fillStyle = '#26180f'; context.fillRect(left + building.width * .44, ground - Math.max(7, building.height * .28), Math.max(4, building.width * .12), Math.max(7, building.height * .28));
      } else {
        context.fillStyle = this.color(presentation.colors.settlement); context.fillRect(left, top, building.width, building.height);
        context.strokeStyle = this.color(presentation.accent, .32 + presentation.awareness * .12); context.lineWidth = 1; context.strokeRect(left, top, building.width, building.height);
        if (snapshot.stage >= 2 && hash01(building.x + civ.seed) > .55) {
          context.strokeStyle = this.color(presentation.accent, .58); context.lineWidth = 2; context.beginPath(); context.moveTo(building.x, top); context.lineTo(building.x, top - Math.max(8, building.height * .12)); context.stroke();
        }
      }
    }
  ...
  private drawFallbackPath(context: CanvasRenderingContext2D, civ: Civilization, presentation: ReturnType<typeof worldPresentation>, time: number): void {
    const path = CivilizationPaths.ensure(civ).dominantPath;
    if (!path || !this.snapshot) return;
    const ground = this.height * .72; const worldWidth = this.snapshot.worldWidth;
    context.strokeStyle = this.color(presentation.accent, .38); context.fillStyle = this.color(presentation.accent, .2); context.lineWidth = 1.5;
    for (let i = 0; i < 10; i++) {
      const x = worldWidth * (.06 + i * .095); const y = ground - 44 - i % 3 * 25;
      context.beginPath();
      if (path === 'collective_mind') {
        context.arc(x, y, 3, 0, Math.PI * 2); context.fill(); if (i) { context.moveTo(x - worldWidth * .095, ground - 44 - (i - 1) % 3 * 25); context.lineTo(x, y); context.stroke(); }
      } else if (path === 'temporal_dominion' || path === 'void_communion' || path === 'post_mortal_civilization') {
        const pulse = path === 'void_communion' ? Math.sin(time * .001 + i) * 3 : 0; context.arc(x, y, 9 + i % 4 * 3 + pulse, 0, Math.PI * 2); context.stroke();
      } else if (path === 'reality_engineering' || path === 'biological_transcendence') {
        context.moveTo(x - 11, y + 9); context.lineTo(x, y - 11); context.lineTo(x + 11, y + 9); context.closePath(); path === 'biological_transcendence' ? context.fill() : context.stroke();
      } else {
        context.rect(x - 10, y - 8, 20, 16); path === 'cosmic_resistance' ? context.fill() : context.stroke();
      }
    }
  }

  private drawDynamic(time: number, civ: Civilization, snapshot: ReturnType<typeof worldSnapshot>, presentation: ReturnType<typeof worldPresentation>): void {
    const context = this.dynamicContext;
    const animationTime = reducedMotion ? 0 : time;
    const worldWidth = snapshot.worldWidth; const ground = this.height * .72;
    context.setTransform(devicePixelRatio, 0, 0, devicePixelRatio, 0, 0); context.clearRect(0, 0, this.width, this.height);
    for (let i = 0; i < snapshot.hazeBands; i++) {
    ...
    for (let i = 0; i < snapshot.trafficCount; i++) {
      const progress = (hash01(civ.seed + i * 47) + animationTime * .00002 * (.5 + hash01(i * 13))) % 1;
      context.fillStyle = this.color(i % 2 ? presentation.accent : presentation.colors.window, .74); context.fillRect(progress * worldWidth, ground + 9 + i % 3 * 5, 6 + snapshot.stage, 2.5);
    }
    for (let i = 0; i < snapshot.aircraftCount; i++) {
      const progress = (hash01(civ.seed + i * 71) + animationTime * .000008 * (.7 + hash01(i * 5))) % 1;
      const x = progress * worldWidth; const y = this.height * (.18 + hash01(i * 23) * .22);
      context.strokeStyle = this.color(presentation.accent, .62); context.beginPath(); context.moveTo(x - 10, y); context.lineTo(x + 10, y); context.stroke();
    }
    for (let i = 0; i < snapshot.beaconCount; i++) {
      const x = worldWidth * (.08 + hash01(civ.seed + i * 97) * .84); const pulse = reducedMotion ? 1 : .7 + Math.sin(animationTime * .003 + i) * .3;
      context.strokeStyle = this.color(presentation.accent, .16 + presentation.awareness * .25 * pulse); context.beginPath(); context.arc(x, ground - 55 - i % 3 * 28, 10 + pulse * 8, 0, Math.PI * 2); context.stroke();
    }
    for (let i = 0; i < snapshot.fractureCount; i++) {
      const x = worldWidth * hash01(civ.seed + i * 61); context.strokeStyle = this.color(0xee6973, .24 + presentation.danger * .42); context.beginPath(); context.moveTo(x, ground + 2); context.lineTo(x + (hash01(i * 11) - .5) * 46, ground + 24 + hash01(i * 17) * 34); context.stroke();
    }
    ...
    if (feedback && this.feedbackStartTime > 0) drawCanvasDecisionImpulse(context,feedback,this.feedbackStartTime,time,this.width,this.height,(value,alpha=1)=>this.color(value,alpha));
    context.setTransform(1, 0, 0, 1, 0, 0);
  }
  ...
  private loop = (time: number): void => {
    ...
    if (key !== this.lastStructuralKey || this.scroll !== this.lastStaticScroll) {
      this.lastStructuralKey = key; this.lastStaticScroll = this.scroll; this.snapshot = snapshot;
  ...
  destroy(): void { cancelAnimationFrame(this.raf); this.staticCanvas.remove(); this.dynamicCanvas.remove(); }
}
...
export function startWorldRenderer(engine: GameEngine, host: HTMLElement): WorldController {
  ...
  let lastStage = -1;
  let cachedSnapshot: ReturnType<typeof worldSnapshot> | null = null;
  let cachedPresentation: ReturnType<typeof worldPresentation> | null = null;
  let cachedBuildings: BuildingShape[] = [];
  let feedbackSequence = 0;
  let feedbackStartTime = 0;
  let activeFeedback: DecisionFeedback | null = null;
  ...
  const activateFallback = (): void => {
    rendererFailed = true;
    try { game?.destroy(true); } catch { /* Renderer teardown must never block the UI. */ }
    game = null; scene = null; layers = null; resetRenderState(); host.replaceChildren();
  ...
  const ensure = (): void => {
    if (engine.state.phase !== 'civilization' || !engine.state.civilization) {
      if (game) { game.destroy(true); game = null; scene = null; layers = null; host.replaceChildren(); }
      fallback?.destroy(); fallback = null; rendererFailed = false; resetRenderState();
      return;
    }
    if (game || fallback) return;
    const PhaserRuntime = (globalThis as any).Phaser;
    if (!PhaserRuntime) { fallback = new FallbackWorld(engine, host); return; }
    ...
    const sceneConfig = {
      create(this: any) {
        try {
          ...
          this.input.on('pointerdown', (pointer: any) => { this.__dragX = pointer.x; this.__startScroll = this.cameras.main.scrollX; });
          ...
          this.input.on('wheel', (_pointer: any, _objects: any, deltaX: number, deltaY: number) => {
            const max = Math.max(0, this.cameras.main.getBounds().width - this.scale.width);
            this.cameras.main.scrollX = Math.max(0, Math.min(max, this.cameras.main.scrollX + deltaX + deltaY * .35));
          });
        } catch {
          if (!rendererFailed) { rendererFailed = true; queueMicrotask(activateFallback); }
        }
      },
      update(this: any, time: number) {
        try {
          const civ = engine.state.civilization;
          if (!civ || !layers || rendererFailed) return;
          ...
          if (!cachedSnapshot || !cachedPresentation) return;
          const feedback = engine.worldImpulse;
          if (feedback && feedback.sequence !== feedbackSequence) {
            feedbackSequence = feedback.sequence; feedbackStartTime = time; activeFeedback = feedback;
          }
  ...
  const onPhaserReady = (): void => {
    if (fallback && (globalThis as any).Phaser) { fallback.destroy(); fallback = null; rendererFailed = false; resetRenderState(); host.replaceChildren(); ensure(); }
  };
  window.addEventListener('phaser-ready', onPhaserReady);
  const unsubscribe = engine.onChange(ensure);
  ensure();
  const resize = new ResizeObserver(() => {
    if (game && scene) { lastStructuralKey = ''; game.scale.resize(Math.max(320, host.clientWidth), Math.max(300, host.clientHeight)); }
  });
  resize.observe(host);

  return {
    nudge(direction: number) {
      if (fallback) return fallback.nudge(direction);
      if (scene) {
    ...
    destroy() {
      unsubscribe(); resize.disconnect(); window.removeEventListener('phaser-ready', onPhaserReady);
      if (game) game.destroy(true);
      fallback?.destroy(); resetRenderState(); host.replaceChildren();
    },
  };
}
