...
/**
 * How the world's atmosphere and its light reach a single building. `fadeColor` is what a distant
 ...
 */
export interface StructureStyle { fadeColor?: number; fade?: number; lightLevel?: number; lightColor?: number; crown?: SkylineCrown; }

/**
 * What the dominant path builds *with*, written into the skyline itself rather than into the palette.
 * Two paths used to differ by their capital motif, a scatter of ambient marks and a hue -- and a hue
 * is exactly what the design says a path identity must not be reducible to. This is the same identity
 * expressed as geometry on the buildings the player is actually looking at: every tall civic and
 * residential solid in the city ends the way its civilization ends things.
 *
 * The names are `identity.ts`'s own `crown` names, so the catalog there stays the single authority on
 * which path builds which way and this module only knows how to draw them.
 */
export type SkylineCrown =
  | 'luminous_core' | 'synchronized_cluster' | 'offset_ring' | 'geometric_frame' | 'living_crown'
  | 'blackout_shield' | 'ordered_block' | 'continuity_beacon' | 'absence_well' | 'nested_crown';
  ...
  if (bodyHeight > 16 && structure.kind !== 'orbital_anchor' && structure.kind !== 'monument') {
    ...
    for (let row = 0; row < rows; row++) {
      for (let column = 0; column < columns; column++) {
        if (hash01(seed + structure.x + row * 17 + column * 131) < .42) continue;
