...
/**
 * How the world's atmosphere reaches a single building. `fadeColor` is what a distant plane fades
 * *into* -- the haze the caller already resolved -- and `lightLevel` is how lit the civilization is,
 * which decides whether a facade carries emissive detail at all. Both are optional so the drawing
 * stays callable with nothing but a palette.
 */
export interface StructureStyle { fadeColor?: number; fade?: number; lightLevel?: number; }
