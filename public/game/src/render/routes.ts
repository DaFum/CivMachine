...
export function roadbedHeight(stage: number): number { return 12 + Math.max(0, stage) * 3; }
/**
 ...
 */
export function roadLaneOffset(stage: number, lane: number, thickness = 0): number {
  const inset = 2;
