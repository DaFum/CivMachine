...
export function drawIdentityLandmarks(surface: DrawSurface, civ: Civilization, settlements: ReadonlyArray<Settlement>, ground: number, accent: number, view: {from:number;to:number}): void {
  ...
  if (capital && identity.tier >= 2 && capital.centerX >= view.from-100 && capital.centerX <= view.to+100) {
    ...
    if (identity.pathId === 'machine_faith') {
      ...
      for(let g=0;g<3;g++) surface.lineStyle(1.4,accent,.6).line(x-12+g*12,ground-18,x-12+g*12,top+15);
    }
      ...
      for(let i=-2;i<=2;i++){
        ...
        if(i<2) surface.lineStyle(1.6,accent,.55).line(x+i*16,nodeY,x+(i+1)*16,top+Math.abs(i+1)*7);
      }
    }
      ...
      for(let row=0;row<4;row++) surface.lineStyle(1.5,accent,.6).strokeRect(x-26+row*4,top-20+row*10,52-row*8,22);
    }
...
function leadingAffinityPath(state: Civilization['pathState'] | undefined): string {
  const affinity = state?.affinity; if (!affinity) return '';
  let best = '', top = 0;
  for (const id of PATH_IDS) { const value = Number(affinity[id] ?? 0); if (value > top) { top = value; best = id; } }
  return best;
}
  ...
  switch (path) {
    ...
    case 'bureaucratic_singularity':
      ...
      for (let i = 0; i < 10; i += step) {
        ...
        if (detailed) surface.lineStyle(1, accent, alpha(.18)).line(x + 4, y + 13, x + 23, y + 13);
      }
      break;
    ...
    case 'void_communion':
      for (let i = 0; i < 7; i += step) {
        ...
        if (detailed) surface.lineStyle(1, accent, alpha(.18)).strokeCircle(x, y, 34);
      }
      break;
