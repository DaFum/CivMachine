 ...
 */
export function spreadPosition(span: number, index: number, offset: number): number {
  let position = 0, denominator = 1, value = Math.max(0, Math.trunc(index));
  while (value > 0) { denominator *= 2; position += (value % 2) / denominator; value = Math.floor(value / 2); }
 ...
 */
const BAYER_4X4: ReadonlyArray<number> = [0, 8, 2, 10, 12, 4, 14, 6, 3, 11, 1, 9, 15, 7, 13, 5];
export function bayerThreshold(cellX: number, cellY: number): number {
