...
const pick=(values:string[],index:number)=>values[((index%values.length)+values.length)%values.length] ?? values[0] ?? text().reports.lore.unknown;
export function speciesProfile(civ:Civilization){
  ...
  const bodyTypes:Readonly<Record<string,string>>=lore.bodyTypes;
