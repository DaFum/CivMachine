...
export class Progression {
  ...
  private static milestone(state:GameState,id:string,award:number,out:string[]){ if(state.meta.progression.milestones[id])return false; state.meta.progression.milestones[id]=true; state.meta.progression.machineInsight+=award; if(award)out.push(`MACHINE INSIGHT +${award}: ${id.replaceAll('_',' ')}`); return true; }
  ...
  static recordCivilizationProgress(state:GameState,civ:{development:number;era:number;stats:{awareness:number}}){ const out:string[]=[]; if(civ.development>=70){this.milestone(state,'development_70',1,out);this.discover(state,'cognition','Cognition',out);} if(civ.era>=1)this.milestone(state,'era_expansion',1,out); if(civ.development>=180)this.milestone(state,'development_180',1,out); if(civ.era>=2)this.milestone(state,'era_transcendence',2,out); if(civ.era>=3)this.milestone(state,'era_apotheosis',2,out); if(civ.development>=340)this.milestone(state,'development_340',2,out); if(civ.stats.awareness>=50)this.milestone(state,'awareness_50',1,out); return this.refresh(state,out); }
  static recordHarvest(state:GameState,record:any){ const out:string[]=[]; if(record.chaotic) state.meta.progression.chaoticHarvestsTotal++; else {state.meta.progression.controlledHarvestsTotal++; const count=state.meta.progression.controlledHarvestsTotal; if(count>=1){this.milestone(state,'controlled_harvest_1',2,out);this.discover(state,'paradox','Paradox',out);} if(count>=2)this.milestone(state,'controlled_harvest_2',2,out);} if(Number(record.development??0)>=180)this.discover(state,'paradox','Paradox',out); return this.refresh(state,out); }
  static recordUniverse(state:GameState){const out:string[]=[];if(state.meta.universesTotal<=1)this.milestone(state,'first_universe',4,out);else{state.meta.progression.machineInsight++;out.push('MACHINE INSIGHT +1: Repeated universe consumption');}this.discover(state,'existence','Existence',out);this.discover(state,'universal_residue','Universal Residue',out);return this.refresh(state,out);}
  static recordMultiverse(state:GameState){const out:string[]=[];this.milestone(state,'first_multiverse',6,out);this.discover(state,'axioms','Axioms',out);return this.refresh(state,out);}
