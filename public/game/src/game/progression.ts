...
export class Progression {
  ...
  static refresh(state:GameState,out:string[]=[]){ const p=state.meta.progression; const insight=this.machineInsight(state); if(p.controlledHarvestsTotal>=2&&insight>=3)this.unlockSystem(state,'directives',out); if(state.machine.civilizationsTotal>=4||insight>=6)this.unlockSystem(state,'universe_prestige',out); if(state.meta.universesTotal>=1)this.unlockSystem(state,'universe_upgrades',out); if(state.meta.universesTotal>=2&&insight>=7)this.unlockSystem(state,'breeding_matrices',out); if(state.meta.universesTotal>=3)this.unlockSystem(state,'multiverse_prestige',out); if(state.meta.multiversesConsumed>=1&&insight>=18)this.unlockSystem(state,'axioms',out); this.refreshKnown(state,'directives',DIRECTIVE_INSIGHT,'knownDirectives',out); this.refreshKnown(state,'breeding_matrices',MATRIX_INSIGHT,'knownBreedingMatrices',out); this.refreshKnown(state,'axioms',AXIOM_KNOWLEDGE,'knownAxioms',out); return out; }
