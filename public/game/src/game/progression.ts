...
export class Progression {
  ...
  static recordCivilizationProgress(state:GameState,civ:{development:number;era:number;stats:{awareness:number}}){ const out:string[]=[]; if(civ.development>=70){this.milestone(state,'development_70',1,out);this.discover(state,'cognition','Cognition',out);} if(civ.era>=1)this.milestone(state,'era_expansion',1,out); if(civ.development>=180)this.milestone(state,'development_180',1,out); if(civ.era>=2)this.milestone(state,'era_transcendence',2,out); if(civ.development>=340)this.milestone(state,'development_340',2,out); if(civ.stats.awareness>=50)this.milestone(state,'awareness_50',1,out); return this.refresh(state,out); }
