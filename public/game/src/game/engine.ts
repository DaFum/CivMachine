...
import type { Civilization, DecisionFeedback, GameState, Layer, ResourceKey, RuntimeBonuses, StorageLike, TacticalActionId, VictoryRecord } from './types.js';
...
export class GameEngine {
  ...
  startCivilization(requestedSeed=0,terminal=false){if(this.state.phase!=='machine')return false;const run=this.state.machine.runBuild;if(!terminal&&this.systemUnlocked('directives')&&run.directiveOfferIds.length&&!run.selectedDirective){this.lastActionFailure='Select one Directive before starting the Civilization.';this.emit();return false;}this.decisionFeedback=null;this.worldImpulse=null;this.lastActionFailure='';const seed=requestedSeed||run.nextCivilizationSeed||mixSeed(Date.now());const selection=this.buildTraitSelection(seed);const usePreview=seed===run.nextCivilizationSeed&&run.previewTraitIds.length>0;const traitIds=usePreview?[...run.previewTraitIds]:selection.ids;const bonuses=this.runtimeBonuses();const setup=terminal?terminalCivilizationSetup():null;const era=setup?setup.era:Math.max(0,Math.min(2,Math.trunc(bonuses.startingEra)));const civ=GameEngine.createCivilizationForTest(seed);civ.terminal=terminal;civ.rngState=selection.rngState;civ.years=setup?setup.years:ERA_YEAR_THRESHOLDS[era]!;civ.era=era;civ.development=setup?setup.development:1+era*80;civ.developmentMultiplier=bonuses.developmentMult;civ.eventTimer=4;civ.stats.stability=bonuses.stabilityMax;civ.stats.stabilityMax=bonuses.stabilityMax;civ.harvestMult={causal_mass:bonuses.causal_massMult,cognition:bonuses.cognitionMult,paradox:bonuses.paradoxMult,existence:bonuses.existenceMult};civ.stabilityDecayMult=bonuses.stabilityDecayMult;civ.eventDelayBonus=bonuses.eventDelay;civ.predictionLevel=bonuses.predictionLevel;civ.directiveId=run.selectedDirective;const objective=objectiveForDirective(civ.directiveId);civ.directiveObjective={id:objective?.id??'',completed:false};
  ...
  developmentRate(){const civ=this.state.civilization;if(!civ)return 0;return developmentGrowthPerSecond(civ,this.upgradeLevel('axiom','axiom_paradox_food'));}
  ...
  runInterventions(){
    const civ=this.state.civilization;const depth=civ?cultivationDepth(civ):0;
    return RUN_INTERVENTIONS.map(definition=>{
      ...
      if(!civ){enabled=false;reason='Start a civilization first.';}
      else if(this.machineInsight()<definition.insight){enabled=false;reason=`Requires Machine Insight ${definition.insight}.`;}
      else if(usesLeft<=0){enabled=false;reason=`${definition.title} is exhausted for this civilization.`;}
      else if(this.currencyAmount(definition.currency)<cost){enabled=false;reason=`Requires ${cost} ${definition.currency.replaceAll('_',' ')}.`;}
  ...
  useTacticalAction(id:TacticalActionId){
    ...
    if(dominant){this.recordDominantPath(dominant);this.post(`DOMINANT CIVILIZATION PATH: ${CivilizationPaths.displayName(dominant).toUpperCase()}`);}
    this.lastActionFailure='';
  ...
  private eventEligible(e:any,civ:Civilization){if(civ.era<Number(e.min_era??0)||civ.era>Number(e.max_era??2))return false;const r=e.requirements??{};if(r.scheduled_only)return false;if(!CivilizationPaths.eventIsEligible(e,civ))return false;const s=civ.stats;if(r.min_attention!=null&&s.attention<Number(r.min_attention))return false;if(r.max_attention!=null&&s.attention>Number(r.max_attention))return false;if(r.min_awareness!=null&&s.awareness<Number(r.min_awareness))return false;if(r.max_awareness!=null&&s.awareness>Number(r.max_awareness))return false;if(r.max_sanity!=null&&s.sanity>Number(r.max_sanity))return false;if(r.min_sanity!=null&&s.sanity<Number(r.min_sanity))return false;if(r.max_stability!=null&&s.stability>Number(r.max_stability))return false;if(r.requires_trait&&!civ.traits.includes(String(r.requires_trait)))return false;if(r.requires_flag&&!civ.flags.includes(String(r.requires_flag)))return false;if(r.required_institution&&!civ.institutions.includes(String(r.required_institution)))return false;if(r.excluded_flag&&civ.flags.includes(String(r.excluded_flag)))return false;if(r.min_development!=null&&civ.development<Number(r.min_development))return false;return true;}
