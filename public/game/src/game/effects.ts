import type { Civilization, ResourceKey, RuntimeBonuses } from "./types.js";

export function clampStats(civ: Civilization) {
  const s = civ.stats;
...
export function applyEffects(
  ...
  for (const key in effects) {
    ...
    if (key === "stability" || key === "awareness" || key === "sanity" || key === "attention") {
      ...
      (civ.stats as any)[key] += amount;
    } else if (key === "entropy")
      civ.tactical.entropy = Math.max(
        0,
        Math.min(100, civ.tactical.entropy + Number(value)),
      );
    else if (key === "control_capacity")
      civ.tactical.controlCapacity = Math.max(
        0,
        Math.min(3, civ.tactical.controlCapacity + Number(value)),
      );
    else if (key === "stability_max") {
      civ.stats.stabilityMax = Math.max(
        1,
        civ.stats.stabilityMax + Number(value),
      );
      civ.stats.stability = Math.min(
        civ.stats.stability,
        civ.stats.stabilityMax,
      );
    } else if (key === "development")
      civ.development = Math.max(1, civ.development + Number(value));
    else if (key === "development_mult")
      civ.developmentMultiplier = Math.max(
        0.2,
        civ.developmentMultiplier + Number(value),
      );
    else if (key === "event_delay") civ.eventDelayBonus += Number(value);
    else if (key === "stability_decay_mult")
      civ.stabilityDecayMult = Math.max(
        0.1,
        civ.stabilityDecayMult * Number(value),
      );
      ...
      const rk = key.slice(13) as ResourceKey;
      if (rk in civ.harvestMult) civ.harvestMult[rk] *= Number(value);
    } else if (key.startsWith("harvest_")) {
      const rk = key.slice(8) as ResourceKey;
      if (rk in civ.harvestBonus) civ.harvestBonus[rk] += Number(value);
    }
  }
  clampStats(civ);
}
