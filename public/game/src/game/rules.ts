 ...
 */
export function upgradeCost(baseCost: number, growth: number, level: number, ladder?: readonly number[]): number {
  const step = Math.max(0, Math.trunc(level) || 0);
  const rate = Math.max(1, growth);
  if (ladder?.length) {
    if (step < ladder.length) return Math.max(1, Math.round(ladder[step]!));
