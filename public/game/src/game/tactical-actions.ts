...
export function ventStabilityCost(uses: number): number {
...
export function predictionMitigation(level: number): number {
 ...
 */
export function probeControlCost(predictionLevel: number): number {
 ...
 * for Depth right now.
 */
export const ACCELERATE_YEARS = [200, 420, 720, 1150] as const;
export const ACCELERATE_DEVELOPMENT = [6, 16, 30, 48] as const;

/** Simulation speed is permanent progression: earned once with Machine Insight, never re-bought. */
export const SIMULATION_SPEED_INSIGHT = { double: 3, quadruple: 10 } as const;

/** The speeds the interface offers, in order. The rail draws every one of them, locked or not. */
export const SIMULATION_SPEED_STEPS = [1, 2, 4] as const;
...
export function maxSimulationSpeed(machineInsight: number): number {
  const insight = Math.max(0, Number(machineInsight) || 0);
  if (insight >= SIMULATION_SPEED_INSIGHT.quadruple) return 4;
...
export function simulationSpeedInsightFor(speed: number): number {
  if (speed >= 4) return SIMULATION_SPEED_INSIGHT.quadruple;
 ...
 */
export function effectiveMaxSimulationSpeed(machineInsight: number, unlockedFloor: number): number {
 ...
 */
export function tacticalSummary(id: TacticalActionId, bonuses: Pick<RuntimeBonuses, 'accelerateYears' | 'accelerateDevelopment'>): string {
  const copy = tacticalActionText(id);
  if (id !== 'accelerate') return copy.summary;
...
export function tacticalRisk(civ: Civilization, id: TacticalActionId): string {
  ...
  if (id === 'accelerate') return fill(reasons.accelerateRisk, { entropy: accelerateEntropyCost(civ.era) });
  ...
  if (id === 'vent') return fill(reasons.ventRisk, { stability: Math.round(ventStabilityCost(civ.tactical.actionUsage.vent)) });
