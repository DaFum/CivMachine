...
const EFFECT_KEYS: ReadonlyArray<string> = [
  'stability', 'awareness', 'sanity', 'attention', 'development', 'entropy', 'stability_max',
];
const effectLabel = (key:string):string => metricName(key) ?? key;
...
function riskVector(effects:any, precisionLevel:number):string {
  ...
  const vectors=Object.entries(effects).filter(([key,value])=>EFFECT_KEYS.includes(key)&&Number(value)!==0).map(([key,value])=>{
    ...
    if(precisionLevel>=5)return `${effectLabel(key)} ${amountLabel(amount)}`;
    if(precisionLevel>=2){
...
function entropyBand(value:number):{index:number;id:string;label:string}{
  const copy=text().ui.viewModel;
  if(value>=100)return {index:4,id:'cascade',label:copy.entropyCascade};
  if(value>=75)return {index:3,id:'critical',label:copy.entropyCritical};
  if(value>=50)return {index:2,id:'fractured',label:copy.entropyFractured};
  if(value>=25)return {index:1,id:'strained',label:copy.entropyStrained};
...
function buildSituation(
  ...
  objective: { title: string; completed: boolean } | null,
  convergenceTargetDepth: number,
  machine: Parameters<typeof machineSituation>[0],
): SituationReport {
  if (!civ || !tactical || !harvest) return machineSituation(machine);
...
function buildMetaInfoViewModel(engine: GameEngine) {
  const state = engine.state;
  return {
...
function buildUpgradesViewModel(engine: GameEngine) {
  return {
...
export function buildViewModel(engine: GameEngine) {
  ...
  const situation = buildSituation(
    ...
    convergenceTargetDepth,
    buildMachineSituationInput(state, engine, directiveRequiredNow, upgrades.machineUpgrades, openMilestone?.title ?? ''),
  );
