...
const visibleKeys=(vm:any,keys:readonly string[])=>keys.filter(key=>vm.visibleResourceKeys.includes(key));
...
function collapsedCard(id:string,title:string,status:string,body:string,cls=''){return `<details class="panel ${cls}" data-disclosure="${esc(id)}"${disclosureAttr(id)}><summary><span>${title}</span>${status?`<small>${status}</small>`:''}</summary>${body}</details>`;}
function sanitizeHTML(html:string){
  const doc=new DOMParser().parseFromString(html,'text/html');
  const bodyGetter=Object.getOwnPropertyDescriptor(Document.prototype,'body')!.get!;
  const body=(bodyGetter.call(doc)??doc.body) as HTMLElement;
  if(!body)return '';
  const elements=Array.from(Element.prototype.querySelectorAll.call(body,'*') as NodeListOf<Element>);
  ...
  for(const node of elements){
    const tagName=(tagNameGetter.call(node) as string).toUpperCase();
    if(FORBIDDEN_TAGS.has(tagName)){
      toRemove.push(node);
    } else {
      const attributes=attributesGetter.call(node) as NamedNodeMap;
      if(attributes){
        for(const attr of Array.from(attributes)){
          const name=(attrNameGetter.call(attr) as string).toLowerCase();
          const val=attrValueGetter.call(attr) as string;
  ...
  for(const el of toRemove)removeElement.call(el);
  return innerHTMLGetter.call(body) as string;
}
...
function revealTutorialAnchor(tutorialLayer:HTMLElement,view:any){
...
const milestoneRegister=(vm:any)=>{const t=text().ui.app;const groups=['CULTIVATION','HARVEST','PATHS','PRESTIGE','CONVERGENCE'];const sections=groups.map(group=>{const entries=vm.milestones.entries.filter((entry:any)=>entry.group===group);if(!entries.length)return '';const insight=(value:number)=>esc(fill(t.insightAward,{amount:value}));const progressText=(entry:any)=>entry.display==='count'
  ...
  const open=entries.filter((entry:any)=>!entry.completed).map((entry:any)=>`<article class="milestone milestone-${esc(entry.display)}"><div><b>${esc(entry.title)}</b><p>${esc(entry.description)}</p></div><div class="milestone-progress"><div class="meter"><i style="width:${pct(entry.current,entry.target)}"></i></div><small>${progressText(entry)} · ${insight(entry.insight)}</small></div></article>`).join('');const done=entries.filter((entry:any)=>entry.completed).map((entry:any)=>`<article class="milestone complete"><b>\u2713 ${esc(entry.title)}</b><small>${insight(entry.insight)}</small></article>`).join('');return `<div class="milestone-group"><span class="panel-kicker">${esc(milestoneGroupLabel(group)??group)}</span>${open}${done}</div>`;}).join('');return collapsedCard('milestones',esc(t.milestoneRegister),`${fmt(vm.milestones.completed)} / ${fmt(vm.milestones.total)}`,`<div class="milestone-register">${explainNote('milestones',vm.explain)}<p class="register-summary">${esc(fill(t.milestoneSummary,{completed:vm.milestones.completed,total:vm.milestones.total}))}</p>${sections}</div>`,'milestone-card');};
...
function renderMachine(machine:HTMLElement,vm:any,engine:GameEngine){
  const t=text().ui.app;
  ...
  const draftBlock=(kicker:string,body:string)=>`<div><span class="panel-kicker">${esc(kicker)}</span>${body}</div>`;
  ...
  replaceIfChanged(machine,`
    ...
    ${card(esc(t.machineUpgrades),`${explainNote('machine_upgrades',vm.explain)}<div class="upgrade-list">${upgrades(engine,vm.machineUpgrades,'machine')}</div>`)}
    ${vm.systems.universeUpgrades?card(esc(t.universeUpgrades),`<div class="upgrade-list">${upgrades(engine,vm.universeUpgrades,'universe')}</div>`):''}
    ${vm.systems.axioms?card(esc(t.axiomUpgrades),`<div class="upgrade-list">${upgrades(engine,vm.axiomUpgrades,'axiom')}</div>`):''}
    ${convergenceCard(vm)}
...
const rewardText=(key:string,details:any,engine:GameEngine)=>`${resourceShort()[key]} ${key==='causal_mass'||engine.resourceDiscovered(key)?fmt(details.rewards[key]):'???'}`;
const rewardSpan=(kind:string,key:string,details:any,engine:GameEngine)=>`<span data-live="harvest-${kind}-${key}">${esc(rewardText(key,details,engine))}</span>`;
...
const harvestActionSurface=(vm:any)=>{
  const c=text().ui.app;
  const h=vm.harvest;
  if(!h)return'';
...
const pressureRail=(vm:any)=>{const t=vm.tactical;if(!t)return'';const c=vm.civilization;const copy=text().ui.app;
...
function renderWorldHudAndDock(worldHud: HTMLElement, worldStatusDock: HTMLElement, c: any, path: any, vm: any, t: any) {
...
function buildRecordDetails(vm: any, c: any, engine: GameEngine, harvest: any, chaotic: any, t: any): string {
  return `<details data-disclosure="harvest-yield"${disclosureAttr('harvest-yield')}><summary>${esc(t.harvestYieldDetail)}</summary>${card('',`${explainNote('harvest_detail',vm.explain)}<p class="panel-note">${esc(t.harvestDetailDescription)}</p><div class="harvest-grid"><div><b>${esc(t.controlled)}</b>${visibleKeys(vm,CONTROLLED_KEYS).map(key=>rewardSpan('controlled',key,harvest,engine)).join('')}</div><div><b>${esc(t.chaotic)}</b>${visibleKeys(vm,CHAOTIC_KEYS).map(key=>rewardSpan('chaotic',key,chaotic,engine)).join('')}<small>${esc(t.chaoticAutomatic)}</small></div></div>`)}</details>
      <details data-disclosure="civ-record"${disclosureAttr('civ-record')}><summary>${esc(t.civilizationRecord)}</summary>${card('',`<ol class="history">${c.history.length?c.history.map((h:string)=>`<li>${esc(h)}</li>`).join(''):`<li>${esc(t.noRecordedHistoryYet)}</li>`}</ol>`)}</details>
      ...
      <details data-disclosure="machine-record"${disclosureAttr('machine-record')}><summary>${esc(t.machineRecord)}</summary>${card('',`<ul id="machine-log-civ" role="log" aria-live="off">${vm.messages.length?vm.messages.map((x:string)=>`<li>${esc(x)}</li>`).join(''):`<li>${esc(t.machineRecordAwaitingActivity)}</li>`}</ul>`)}</details>`;
}

function renderCivilization(worldHud:HTMLElement,worldStatusDock:HTMLElement,civPanels:HTMLElement,worldShell:HTMLElement,vm:any,engine:GameEngine,stateCtx:{lastFeedbackSequence:number;impactTimer:number}){
  const c=vm.civilization;if(!c)return;const t=text().ui.app;const event=vm.event;const path=c.path;const harvest=vm.harvest.controlled;const chaotic=vm.harvest.chaotic;
  ...
  renderWorldHudAndDock(worldHud, worldStatusDock, c, path, vm, t);
  ...
  const recordDetails=buildRecordDetails(vm, c, engine, harvest, chaotic, t);
  ...
  if(vm.feedback&&vm.feedback.sequence!==stateCtx.lastFeedbackSequence){
    stateCtx.lastFeedbackSequence=vm.feedback.sequence;
    ...
    window.clearTimeout(stateCtx.impactTimer);
...
function refreshCivilizationLive(civPanels:HTMLElement,worldHud:HTMLElement,worldStatusDock:HTMLElement,vm:any,engine:GameEngine){
  ...
  for(const key of visibleKeys(vm,CONTROLLED_KEYS))setText(`[data-live="harvest-controlled-${key}"]`,rewardText(key,vm.harvest.controlled,engine));
  for(const key of visibleKeys(vm,CHAOTIC_KEYS))setText(`[data-live="harvest-chaotic-${key}"]`,rewardText(key,vm.harvest.chaotic,engine));
  ...
  const group=(worldStatusDock?worldStatusDock.querySelector<HTMLElement>('.cascade-group'):null)??worldHud.querySelector<HTMLElement>('.world-mobile-strip');
  if(group)for(const state of ['building','closing','harvest','cascading'])group.classList.toggle(`urgency-${state}`,vm.harvest.urgency.state===state);
...
export function createGameUI(engine:GameEngine,world:WorldController){
  ...
  const worldStatusDock=document.querySelector('#world-status-dock') as HTMLElement;
  ...
  const stateCtx={lastFeedbackSequence:0,impactTimer:0};
  ...
  const render=()=>{
    ...
    if(vm.phase==='victory')renderVictory(victoryView,vm);
    if(vm.phase==='machine'){
      currentCivilizationKey='';
      renderMachine(machine,vm,engine);
    }else{
      const nextKey=civilizationRenderKey(vm);
      if(nextKey!==currentCivilizationKey){
        renderCivilization(worldHud,worldStatusDock,civPanels,worldShell,vm,engine,stateCtx);
        currentCivilizationKey=nextKey;
      }else{
        refreshCivilizationLive(civPanels,worldHud,worldStatusDock,vm,engine);
      }
    }
    ...
    if(log&&log.parentElement)log.parentElement.classList.toggle('is-hidden',vm.phase!=='machine');
    const civLog=civPanels.querySelector('#machine-log-civ');
    if(civLog)replaceIfChanged(civLog as HTMLElement,logItems);
    ...
    if(stepKey!==lastTutorialStepId){lastTutorialStepId=stepKey;revealTutorialAnchor(tutorialLayer,vm.tutorial);}
    if(explainToggle){
