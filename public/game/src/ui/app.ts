...
export function createGameUI(engine:GameEngine,world:WorldController){
  ...
  function renderCivilization(vm:any){const c=vm.civilization;if(!c)return;const event=vm.event;const path=c.path;const harvest=vm.harvest.controlled;const chaotic=vm.harvest.chaotic;
    ...
    const reserveCard=vm.machineReserve.length?card('Machine Reserve',`<p class="panel-note">Commit banked resources to the running civilization. Each use triples the price, and the price rises with the depth already reached.</p><div class="reserve-actions">${vm.machineReserve.map((entry:any)=>`<div class="tactical-action-wrap"><button data-action="reserve" data-id="${esc(entry.id)}" aria-describedby="reserve-reason-${esc(entry.id)}" ${entry.enabled?'':'disabled'}><span>${esc(entry.title)}</span><b>${esc(entry.summary)}</b><small>COST ${fmt(entry.cost)} ${esc(entry.currency.replaceAll('_',' '))} · ${entry.usesLeft} OF ${entry.maxUses} LEFT</small></button><span id="reserve-reason-${esc(entry.id)}" class="tactical-reason">${esc(entry.reason)}</span></div>`).join('')}</div>`,'machine-reserve'):'';
    replaceIfChanged(civPanels,`${tacticalRail(vm)}${decisionFeedback(vm.feedback)}${eventCard}${objectiveCard}${reserveCard}
      ...
      <details><summary>Harvest Projection</summary>${card('',`<div class="harvest-grade"><span>HARVEST GRADE</span><b>${esc(harvest.grade.toUpperCase())}</b><small>×${harvest.rewardMultiplier.toFixed(2)} yield · +${harvest.credits} Cultivation Credit${harvest.credits===1?'':'s'}${harvest.objectiveCompleted?' · OBJECTIVE BONUS ACTIVE':''}</small><small>CULTIVATION DEPTH <b data-live="depth">${vm.harvest.depth.toFixed(1)}</b>${vm.harvest.nextBand?` · NEXT: ${esc(vm.harvest.nextBand.label.toUpperCase())} AT DEPTH ${vm.harvest.nextBand.depthNeeded} FOR ×${vm.harvest.nextBand.yieldMultiplier.toFixed(2)}`:' · DEEPEST BAND REACHED'}</small></div><div class="harvest-grid"><div><b>CONTROLLED</b><span>Causal ${fmt(harvest.rewards.causal_mass)}</span><span>Cognition ${engine.resourceDiscovered('cognition')?fmt(harvest.rewards.cognition):'???'}</span><span>Paradox ${engine.resourceDiscovered('paradox')?fmt(harvest.rewards.paradox):'???'}</span><span>Existence ${engine.resourceDiscovered('existence')?fmt(harvest.rewards.existence):'???'}</span></div><div><b>CHAOTIC</b><span>Causal ${fmt(chaotic.rewards.causal_mass)}</span><span>Paradox ${engine.resourceDiscovered('paradox')?fmt(chaotic.rewards.paradox):'???'}</span><small>Automatic at zero Stability; Premature collapses retain a salvage floor.</small></div></div>`)}</details>
