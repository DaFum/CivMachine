...
export function createGameUI(engine:GameEngine,world:WorldController){
  ...
  function bindActions(){document.querySelectorAll<HTMLElement>('[data-action]').forEach(el=>{if(el.dataset.bound)return;el.dataset.bound='1';el.addEventListener('click',()=>{const a=el.dataset.action;switch(a){case'start':engine.startCivilization();break;case'upgrade':engine.purchaseUpgrade(el.dataset.layer as any,el.dataset.id!);break;case'directive':engine.selectDirective(el.dataset.id!);break;case'matrix':engine.selectBreedingMatrix(el.dataset.id!);break;case'universe':engine.consumeUniverse();break;case'multiverse':engine.consumeMultiverse();break;case'choice':engine.chooseEvent(Number(el.dataset.index));break;case'tactical':engine.useTacticalAction(el.dataset.id as any);break;case'reroll':engine.rerollEvent();break;case'speed':engine.setSimulationSpeed(Number(el.dataset.speed));break;case'harvest':engine.harvest(false);break;case'chaos':engine.harvest(true);break;case'abandon':engine.returnToMachineWithoutReward();break;case'pan':world.nudge(Number(el.dataset.dir));break;}});});}
