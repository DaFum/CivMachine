...
export function createGameUI(engine:GameEngine,world:WorldController){
  ...
  const pressureRail=(vm:any)=>{const t=vm.tactical;if(!t)return'';const c=vm.civilization;const copy=text().ui.app;
    ...
    return `<details class="panel tactical-rail pressure-rail entropy-${esc(t.entropyBand.id)}${focusClass(vm,'.pressure-rail')}" data-disclosure="pressure-details"${disclosureAttr('pressure-details')}><summary>${esc(copy.pressureHarvestDetails??copy.pressureHarvest)} <small data-live="pressure-summary">ENT ${t.entropy.toFixed(1)} · ${esc(t.entropyBand.label)} · CASCADE ${t.secondsToCascade.toFixed(0)}s</small></summary>${explainNote('pressure_rail',vm.explain)}<div class="entropy-readout"><span>${esc(text().ui.viewModel.metrics.entropy.toUpperCase())} // <b data-live="entropy-band">${esc(t.entropyBand.label)}</b></span><strong data-live="entropy-value">${t.entropy.toFixed(1)}</strong><div class="entropy-meter"><i data-live="entropy-meter" style="width:${pct(t.entropy)}"></i></div><small>${readout}</small><small class="cascade-eta">${fill(copy.cascadeCurrentCourse,{seconds:`<b data-live="cascade-eta">${t.secondsToCascade.toFixed(0)}s</b>`})}</small></div>${harvestReadout(vm)}<section class="harvest-actions${focusClass(vm,'.harvest-actions')}"><button class="primary big" data-action="harvest">${esc(copy.controlledHarvest)}</button><div class="harvest-actions-secondary"><button class="danger" data-action="chaos">${esc(copy.forceChaoticHarvest)}</button><button class="ghost" data-action="abandon">${esc(copy.abandonWithoutReward)}</button></div></section></details>`;};
  function renderCivilization(vm:any){const c=vm.civilization;if(!c)return;const t=text().ui.app;const event=vm.event;const path=c.path;const harvest=vm.harvest.controlled;const chaotic=vm.harvest.chaotic;
    ...
    if(worldStatusDock){
      replaceIfChanged(worldStatusDock,`<div class="world-status-strip${focusClass(vm,'.world-state-strip')}"><div class="status-group"><span title="${esc(abbreviationTitle('ERA'))}">ERA <b data-live="world-era">${esc(eraLabel(c.era))}</b></span><span title="${esc(abbreviationTitle('DEV'))}">DEV <b data-live="world-development">${c.development.toFixed(0)}</b></span><span title="${esc(abbreviationTitle('STB'))}">STB <b data-live="world-stability">${c.stats.stability.toFixed(0)}</b></span><span title="${esc(abbreviationTitle('SAN'))}">SAN <b data-live="world-sanity">${c.stats.sanity.toFixed(0)}</b></span></div><div class="status-group"><span title="${esc(abbreviationTitle('AWR'))}">AWR <b data-live="world-awareness">${c.stats.awareness.toFixed(0)}</b></span><span title="${esc(abbreviationTitle('ATT'))}">ATT <b data-live="world-attention">${c.stats.attention.toFixed(0)}</b></span><span title="${esc(abbreviationTitle('ENT'))}">ENT <b data-live="world-entropy">${vm.tactical.entropy.toFixed(0)}</b></span></div><div class="status-group cascade-group urgency-${esc(vm.harvest.urgency.state)}"><span>CASCADE <b data-live="strip-cascade">${vm.tactical.secondsToCascade.toFixed(0)}s</b></span><span class="strip-call" data-live="strip-call">${esc(urgencyShort(vm.harvest))}</span></div></div>`);
    }
