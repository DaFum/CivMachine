...
function renderCivilization(worldHud:HTMLElement,worldStatusDock:HTMLElement,civPanels:HTMLElement,worldShell:HTMLElement,vm:any,engine:GameEngine,stateCtx:{lastFeedbackSequence:number;impactTimer:number}){const c=vm.civilization;if(!c)return;const t=text().ui.app;const event=vm.event;const path=c.path;const harvest=vm.harvest.controlled;const chaotic=vm.harvest.chaotic;
