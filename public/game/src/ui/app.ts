...
export function createGameUI(engine:GameEngine,world:WorldController){
  ...
  function refreshCivilizationLive(vm:any){
    ...
    for(const key of visibleKeys(vm,CONTROLLED_KEYS))setText(`[data-live="harvest-controlled-${key}"]`,rewardText(key,vm.harvest.controlled));
    for(const key of visibleKeys(vm,CHAOTIC_KEYS))setText(`[data-live="harvest-chaotic-${key}"]`,rewardText(key,vm.harvest.chaotic));
