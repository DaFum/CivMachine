 ...
 */
function drawCrown(surface, crown, x, top, width, height, accent, body, lightLevel) {
    ...
    switch (crown) {
        ...
        case 'synchronized_cluster':
            ...
            for (let node = -1; node <= 1; node++)
                surface.fillStyle(accent, .5).fillCircle(x + node * half * .6, top - half * .3, Math.max(1.3, half * .14));
            break;
        case 'offset_ring':
        ...
        case 'living_crown':
            ...
            for (let lobe = -1; lobe <= 1; lobe++)
                surface.fillStyle(mixColor(accent, 0x8ee66b, .4), .34).fillCircle(x + lobe * half * .34, top - half * (.5 + Math.abs(lobe) * -.14), Math.max(1.8, half * .2));
            break;
