...
export function drawConsequenceImpact(surface, feedback, startTime, time, width, height, accent, reducedMotion, scroll = 0, worldWidth = width, settlements = []) {
    ...
    if (impact.kind === 'containment') {
        ...
        if (front > 15)
            surface.lineStyle(Math.max(.5, stroke * .6), color, fade * .42).strokeCircle(cx, cy, front * .72);
 ...
 */
export function drawPhaseTransitionImpact(surface, from, to, startTime, time, width, height, accent, reducedMotion, anchors = []) {
    ...
    const lit = [0, 1, 2].map(bar => anchors[bar % anchors.length] ?? { x: width * (.24 + bar * .26), crown: 90 });
