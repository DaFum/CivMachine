...
export function ridgePoints(view, worldWidth, baseY, step, amplitude, wavelength, seed, detail) {
...
function shelfHeightAt(x, amplitude, wavelength, seed) {
 ...
 */
function drawShelfShadows(surface, presentation, level, shelf, shelfBase, shelfHeight, plane, span, view) {
...
function drawShelfCrestAndContour(surface, presentation, shelf, shelfBase, shelfHeight, shelfPoints, worldWidth, wavelength, shelfSeed, view) {
    const colors = presentation.colors;
    ...
    surface.lineStyle(1, mixColor(colors.skyHorizon, colors.groundNear, .35 + shelf * .2), .2 - shelf * .045)
        .strokePoly(shelfPoints.slice(1, -1).map(([x, y]) => [x + LIGHT_FROM_X * .5, y + LIGHT_FROM_Y * .5]));
    return crestColor;
}
function drawShelfStipple(surface, crestColor, shelfBase, shelfHeight, wavelength, shelfSeed, worldWidth, span, nominalSpan, view) {
    ...
    for (let cell = firstCell; cell <= lastCell && drawn < MAX_DITHER_CELLS; cell++) {
        ...
        for (let row = 0; row < rows && drawn < MAX_DITHER_CELLS; row++) {
            ...
            if (bayerThreshold(cell, Math.floor(y / DITHER_CELL)) > coverage)
                continue;
...
export function drawGroundShelves(surface, presentation, seed, worldWidth, horizon, plane, view, nominalSpan) {
