...
export function drawCreature(surface, profile, caste, x, groundY, scale, phase, accent) {
    ...
    if (caste === 'augmented' || profile.glow > .3)
        surface.fillStyle(accent, .08 + profile.glow * .1).fillCircle(x, bodyTop + size * .45, size * .42);
    ...
    if (profile.features.includes('cap'))
        surface.fillStyle(mixColor(profile.bodyColor, 0xffffff, .3), .95).fillTriangle(x - size * .42, bodyTop + headRadius, x, bodyTop - size * .18, x + size * .42, bodyTop + headRadius);
    if (profile.features.includes('antenna'))
        surface.lineStyle(Math.max(1, size * .1), accent, .8).line(x, bodyTop, x + swing * .6, bodyTop - size * .5);
    if (profile.features.includes('crystal'))
        surface.fillStyle(accent, .6).fillTriangle(x - bodyWidth * .7, bodyTop + size * .34, x - bodyWidth * .2, bodyTop - size * .1, x + bodyWidth * .3, bodyTop + size * .34);
    if (profile.features.includes('smoke'))
        surface.fillStyle(profile.bodyColor, .22).fillCircle(x - swing, bodyTop - size * .3, size * .34);
    if (profile.features.includes('hollow'))
        surface.fillStyle(accent, .85).fillCircle(x, bodyTop + headRadius * .6, headRadius * .38);
    // The halo only reads above ~7px; below that it collapsed into a blob around the head.
    if (caste === 'augmented' && size >= 7)
        surface.lineStyle(1, accent, .6).strokeCircle(x, bodyTop - size * .16, headRadius * .95);
    if (caste === 'labourer')
        surface.lineStyle(Math.max(1, size * .1), outline, .85).line(x + bodyWidth * .6, groundY - size * .5, x + bodyWidth * .6, groundY);
}
