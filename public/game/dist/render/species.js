...
export function speciesProfile(civ) {
    ...
    let spec;
    for (const [traitId, candidate] of TRAIT_ARCHETYPES)
        if (traits.has(traitId)) {
            spec = candidate;
            break;
        }
    if (!spec)
        spec = SEED_ARCHETYPES[Math.min(SEED_ARCHETYPES.length - 1, Math.trunc(hash01(civ.seed * 7 + 3) * SEED_ARCHETYPES.length))];
...
export function drawCreature(surface, profile, caste, x, groundY, scale, phase, accent) {
    ...
    if (caste === 'augmented' || profile.glow > .3)
        surface.fillStyle(accent, .12 + profile.glow * .16).fillCircle(x, bodyTop + size * .4, size * .78);
    ...
    if (profile.features.includes('cap'))
        surface.fillStyle(mixColor(profile.bodyColor, 0xffffff, .3), .9).fillTriangle(x - size * .42, bodyTop + headRadius, x, bodyTop - size * .18, x + size * .42, bodyTop + headRadius);
    if (profile.features.includes('antenna'))
        surface.lineStyle(Math.max(1, size * .1), accent, .7).line(x, bodyTop, x + swing * .6, bodyTop - size * .5);
    if (profile.features.includes('crystal'))
        surface.fillStyle(accent, .5).fillTriangle(x - bodyWidth * .7, bodyTop + size * .34, x - bodyWidth * .2, bodyTop - size * .1, x + bodyWidth * .3, bodyTop + size * .34);
    if (profile.features.includes('smoke'))
        surface.fillStyle(profile.bodyColor, .2).fillCircle(x - swing, bodyTop - size * .3, size * .4);
    if (profile.features.includes('hollow'))
        surface.fillStyle(accent, .75).fillCircle(x, bodyTop + headRadius * .6, headRadius * .34);
    if (caste === 'augmented')
        surface.lineStyle(Math.max(1, size * .08), accent, .55).strokeCircle(x, bodyTop - size * .12, headRadius * 1.5);
    if (caste === 'labourer')
        surface.lineStyle(Math.max(1, size * .1), mixColor(profile.bodyColor, 0x000000, .35), .8).line(x + bodyWidth * .6, groundY - size * .5, x + bodyWidth * .6, groundY);
}
