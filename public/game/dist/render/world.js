...
import { dynamicFrameIntervalMs, qualityFactors, RenderQualityController } from './quality.js';
...
import { ROAD_TOP_OFFSET, roadbedHeight, roadLaneOffset, routeFlowMarks, routeInBand, routeOffsetAt, routePointAt, routePolyline, tradeRoutes } from './routes.js';
 ...
 * will hand out.
 */
function visibleBand(worldWidth, width, scroll, parallax) {
...
function drawCloudStrata(surface, scene, height, view, skyReach) {
    ...
    for (let deck = 0; deck < decks.length && drawn < MAX_CLOUD_BANKS; deck++) {
        ...
        for (let index = first; index <= last && drawn < MAX_CLOUD_BANKS; index++) {
            ...
            if (hash01(civ.seed * 3 + deck * 137 + index * 61) > .36 + deck * .06 + presentation.bands.awareness * .053 - presentation.bands.entropy * .08)
                continue;
 ...
 */
function drawSkyGradientsAndFront(surface, scene, height, view, span, horizon) {
    const { civ, presentation } = scene;
    const colors = presentation.colors;
    // 1. The sky itself
...
function drawSkyStarsAndBody(surface, scene, height, view, skyReach) {
    const { civ, presentation } = scene;
    const colors = presentation.colors;
    // 3. Star field
...
function drawSkyHorizonAndObserver(surface, scene, height, view, span, horizon, skyReach) {
    const { civ, presentation } = scene;
    const colors = presentation.colors;
    // 5. Horizon illumination
...
/**
 * The sky half of the cached static layer.
 */
function drawSkyContent(surface, scene, width, height, view) {
    ...
    drawSkyGradientsAndFront(surface, scene, height, view, span, horizon);
    drawSkyStarsAndBody(surface, scene, height, view, skyReach);
    drawSkyHorizonAndObserver(surface, scene, height, view, span, horizon, skyReach);
}
/**
 ...
 */
function drawMountainRanges(surface, scene, horizon, span, view, terrainReach, midBase, midSeed, farAmplitude, midAmplitude) {
...
function drawTerrainGroundAndShelves(surface, scene, width, height, horizon, span, view, nearAmplitude) {
    const { civ, snapshot, presentation } = scene;
    const worldWidth = snapshot.worldWidth;
    const colors = presentation.colors;
...
function drawEntropyFissures(surface, scene, horizon, view, terrainReach, entropyBand) {
    const { civ, presentation } = scene;
    const colors = presentation.colors;
...
/**
 * The terrain half of the cached static layer.
 */
function drawTerrainContent(surface, scene, width, height, view) {
    const { snapshot, presentation } = scene;
    const worldWidth = snapshot.worldWidth;
    ...
    const midSeed = scene.civ.seed * 7 + 29;
    drawMountainRanges(surface, scene, horizon, span, view, terrainReach, midBase, midSeed, farAmplitude, midAmplitude);
    drawTerrainGroundAndShelves(surface, scene, width, height, horizon, span, view, nearAmplitude);
    if (entropyBand >= 2) {
        drawEntropyFissures(surface, scene, horizon, view, terrainReach, entropyBand);
    }
}
...
function drawOutskirt(surface, prop, ground, presentation) {
    ...
    if (prop.kind === 'field') {
        ...
        for (let row = 0; row < 3; row++)
            surface.lineStyle(1, mixColor(colors.groundNear, 0x6f9c55, .3), .18).line(x - width, ground + 3 + row * 3 * scale, x + width, ground + 3 + row * 3 * scale);
    }
...
function drawTradeNetwork(surface, scene, ground, view) {
    ...
    if (stage > 0) {
        ...
        for (const route of scene.routes) {
            if (!routeInBand(route, view.from, view.to))
                continue;
            const spine = routePolyline(route, view.from, view.to);
            if (spine.length < 2)
                continue;
            const near = spine.map(([x, offset]) => [x, ground + ROAD_TOP_OFFSET + offset]);
            const bed = [...near];
        ...
        if (stage >= 2)
            surface.lineStyle(2, presentation.accent, .24).line(view.from, ground - 9, view.to, ground - 9);
        if (stage >= 4)
        ...
        for (const settlement of settlements) {
            ...
            if (trackFrom > view.to || trackFrom + trackSpan < view.from)
                continue;
    ...
    for (const settlement of settlements) {
        ...
        if (identity.frame !== 'none' && identity.tier >= 2) {
            ...
            if (settlement.centerX + frameReach >= view.from && settlement.centerX - frameReach <= view.to) {
                drawSettlementFrame(surface, identity.frame, { centerX: settlement.centerX, radius: settlement.radius, crown, seed: settlement.lightPhase * Math.PI * 2 }, ground, presentation.accent, identity.tier, colors.lightSpill);
            }
        }
 ...
 */
function drawMoodWash(surface, scene, live, view, height) {
...
function drawParticles(surface, scene, snapshot, presentation, height, view, time, reducedMotion) {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        if (posX < view.from || posX > view.to)
            continue;
...
export function drawHazeBands(surface, snapshot, presentation, width, height, animationTime, view, reducedMotion) {
    ...
    const strata = [[0, .22, .28], [.22, .5, .72], [.5, .78, 1], [.78, 1, .46]];
    const taperSteps = 2;
    ...
    for (let i = 0; i < hazeBands; i++) {
        ...
        for (const offset of [0, -worldWidth, worldWidth]) {
            ...
            if (bx > view.to || bx + bandWidth < view.from)
                continue;
            ...
            for (const [from, to, weight] of strata) {
                ...
                if (coreTo > coreFrom)
                    surface.fillStyle(presentation.colors.haze, opacity * weight).fillRect(coreFrom, sy, coreTo - coreFrom, sh);
                for (let step = 0; step < taperSteps; step++) {
                    ...
                    if (leftTo > leftFrom)
                        surface.fillStyle(presentation.colors.haze, alpha).fillRect(leftFrom, sy, leftTo - leftFrom, sh);
                    ...
                    if (rightTo > rightFrom)
                        surface.fillStyle(presentation.colors.haze, alpha).fillRect(rightFrom, sy, rightTo - rightFrom, sh);
                }
            }
        }
    }
 ...
 */
function drawWindowLights(surface, scene, snapshot, presentation, ground, animationTime, view, share, glowDetail, reducedMotion, onScreen) {
    const civ = scene.civ;
    const lightLevel = presentation.lightLevel;
    ...
    for (const settlement of onScreen) {
        ...
        for (let index = 0; index < inView.length; index += stride) {
            ...
            for (let w = 0; w < windows; w++) {
                ...
                surface.fillStyle(windowColor, intensity).fillRect(structure.x - structure.width * .34 + column * structure.width * .56, effGround - structure.height + 6 + slot * (structure.height * .78 / rows), Math.max(1.6, 2.2 + snapshot.stage * .3), Math.max(2, 3 + snapshot.stage * .2));
            }
...
function drawMicroLights(surface, scene, presentation, ground, animationTime, view, windowFraction, reducedMotion, onScreen) {
    const civ = scene.civ;
    const lightLevel = presentation.lightLevel;
    const windowColor = presentation.colors.window;
    const spill = presentation.colors.lightSpill;
...
function drawStreetLamps(surface, scene, snapshot, presentation, ground, animationTime, view, glowDetail, reducedMotion) {
    if (snapshot.stage < 1)
        return;
    ...
    for (let lamp = firstLamp; lamp * 118 <= view.to; lamp++) {
        ...
        let near = false;
        for (const settlement of settlements)
            if (Math.abs(settlement.centerX - x) < settlement.radius + 90) {
                near = true;
                break;
            }
        if (!near)
...
export function drawCityLights(surface, scene, snapshot, presentation, ground, animationTime, view, windowFraction, glowDetail, reducedMotion) {
    ...
    drawWindowLights(surface, scene, snapshot, presentation, ground, animationTime, view, share, glowDetail, reducedMotion, onScreen);
    drawMicroLights(surface, scene, presentation, ground, animationTime, view, windowFraction, reducedMotion, onScreen);
    drawStreetLamps(surface, scene, snapshot, presentation, ground, animationTime, view, glowDetail, reducedMotion);
}
/**
...
export function drawRouteFlow(surface, scene, presentation, ground, animationTime, view, reducedMotion, glowDetail) {
    ...
    for (let index = 0; index < visible.length; index++) {
        ...
        for (let mark = 0; mark < count; mark++) {
            ...
            if (Math.max(from.x, to.x) < view.from || Math.min(from.x, to.x) > view.to)
                continue;
            ...
            if (glowDetail > 0)
                surface.fillStyle(color, Math.min(.4, alpha * .5 + .1) * glowDetail).fillCircle(to.x, headY, FLOW_HEAD_RADIUS * 1.9);
...
function drawTraffic(surface, scene, snapshot, presentation, ground, height, animationTime, view, agentFraction, reducedMotion) {
    ...
    for (const launch of plan.launches) {
        ...
        if (cycle > .42 || launch.x < view.from || launch.x > view.to)
            continue;
        const rise = cycle / .42;
...
function drawDynamicContent(surface, scene, snapshot, presentation, width, height, time, tracker, view, tier) {
    ...
    if (identity.frame !== 'none' && identity.tier >= 2) {
        ...
        const framed = scene.settlements.filter(settlement => settlement.centerX + FRAME_MAX_REACH >= view.from && settlement.centerX - FRAME_MAX_REACH <= view.to);
...
class CanvasWorld {
    constructor(engine, host) {
        ...
        this.loop = (time) => {
            ...
            if (time - this.lastFrame < dynamicFrameIntervalMs(this.quality.tier, this.quality.averageCostMs, currentReducedMotion))
                return;
            this.lastFrame = time;
