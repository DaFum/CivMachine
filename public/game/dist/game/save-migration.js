...
const SAVE_MIGRATIONS_BY_FROM = new Map(SAVE_MIGRATIONS.map(step => [step.from, step]));
...
export function normalizeState(raw, log) {
    ...
    if (unlocked !== undefined) {
        ...
        if (repaired !== unlocked)
            log.repair('meta.progression.simulationSpeedUnlocked', repaired);
