...
export const TUTORIAL_STEPS = [
    ...
    },
    {
        ...
        where: 'The status dock under the world: ERA, DEV, STB, SAN, AWR, ATT, ENT. Drag the world to look around it.',
        why: 'It means nothing on screen is decoration. If the world changes, a number changed, and the dock says which.',
        action: '',
        anchor: '.world-status-strip',
        phase: 'civilization',
        requires: '',
    },
    {
...
const STEP_INDEX = Object.fromEntries(TUTORIAL_STEPS.map((step, index) => [step.id, index]));
export function tutorialStepById(id) {
    const index = STEP_INDEX[id];
    return index === undefined ? null : TUTORIAL_STEPS[index];
}
export function newTutorialState(status = 'pending') {
...
export function normalizeTutorialState(state) {
    const raw = state ?? newTutorialState();
    const status = raw.status === 'active' || raw.status === 'completed' || raw.status === 'skipped' ? raw.status : 'pending';
...
export function recordTutorialFact(state, fact) {
    if (state.observed.includes(fact))
        return false;
 ...
 */
export function liveTutorialFacts(civ) {
    if (!civ)
        return [];
    const facts = ['run_started'];
    if (civ.eventChoices > 0)
        facts.push('intervention_resolved');
    if (Object.values(civ.tactical.actionUsage ?? {}).some(count => (Number(count) || 0) > 0))
        facts.push('tactical_used');
    return facts;
}
export function stepCleared(state, step) {
...
export function advanceTutorial(state) {
    ...
    while (index < TUTORIAL_STEPS.length && stepCleared(state, TUTORIAL_STEPS[index]))
        index++;
    ...
    if (index === startedAt && state.stepId === TUTORIAL_STEPS[index].id)
        return false;
    state.stepId = TUTORIAL_STEPS[index].id;
    return true;
}
export function acknowledgeStep(state) {
    ...
    if (!step || step.requires)
        return false;
    if (!state.acknowledged.includes(step.id))
        state.acknowledged.push(step.id);
    advanceTutorial(state);
    return true;
}
export function tutorialView(state, phase) {
    const step = currentStep(state);
