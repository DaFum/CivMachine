...
const seconds = (value) => (Number.isFinite(value) ? `${Math.max(0, Math.round(value))}s` : text().ui.app.noLimit);
...
export function machineSituation(input) {
    ...
    if (input.affordableUpgrades > 0) {
        return {
            id: 'spend_bank',
            severity: 'calm',
            headline: fill(count(input.affordableUpgrades) === 1 ? copy.spend_bank.headlineOneUpgrade : copy.spend_bank.headlineManyUpgrades, { count: count(input.affordableUpgrades) }),
