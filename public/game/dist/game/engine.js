...
export class GameEngine {
    ...
    emit() { for (const fn of this.listeners)
        fn(); }
    post(msg) { this.messages.unshift(msg); this.messages = this.messages.slice(0, 80); }
    save() { if (!this.autosave)
        return; this.state.saveVersion = SAVE_VERSION; this.storage.setItem(SAVE_KEY, JSON.stringify(this.state)); }
    load() { try {
    ...
    static baseBonuses() { return { stabilityMax: 100, predictionLevel: 0, developmentMult: 1, causal_massMult: 1, cognitionMult: 1, paradoxMult: 1, existenceMult: 1, awarenessGainMult: 1, sanityLossMult: 1, attentionGainMult: 1, stabilityLossMult: 1, stabilityDecayMult: 1, eventDelay: 0, startingEra: 0, extraTraits: 0, allHarvestMult: 1, chaoticRetention: .4, containmentRating: 0, entropyGainMult: 1, controlRecharge: 1, accelerateYears: 200, accelerateTimer: 8, gradeRewardMult: 1 }; }
    ...
    upgradeById(layer, id) { return this.catalog(layer).find((u) => u.id === id) ?? null; }
    catalog(layer) { return layer === 'machine' ? this.machineUpgrades : layer === 'universe' ? this.universeUpgrades : this.axiomUpgrades; }
    levels(layer) { return layer === 'machine' ? this.state.machine.upgradeLevels : layer === 'universe' ? this.state.meta.universeUpgradeLevels : this.state.meta.axiomLevels; }
    upgradeLevel(layer, id) { return Math.max(0, Number(this.levels(layer)[id] ?? 0)); }
    upgradeCost(layer, id) { const d = this.upgradeById(layer, id); return d ? upgradeCost(Number(d.base_cost), Number(d.growth), this.upgradeLevel(layer, id)) : 0; }
    currencyAmount(currency) { if (RESOURCE_KEYS.includes(currency))
        return this.state.machine.currencies[currency]; if (currency === 'universal_residue')
        return this.state.meta.universalResidue; if (currency === 'axioms')
        return this.state.meta.axioms; return 0; }
    spendCurrency(currency, amount) { if (RESOURCE_KEYS.includes(currency))
    ...
    systemUnlocked(id) { return Progression.systemUnlocked(this.state, id); }
    resourceDiscovered(id) { return Progression.resourceDiscovered(this.state, id); }
    machineInsight() { return Progression.machineInsight(this.state); }
    availableDirectives() { return this.directives.filter((d) => this.state.machine.runBuild.directiveOfferIds.includes(d.id)); }
    availableMatrices() { return this.matrices.filter((d) => this.state.meta.progression.knownBreedingMatrices.includes(d.id)); }
    selectDirective(id) { const r = this.state.machine.runBuild; if (!this.systemUnlocked('directives') || r.directiveLocked || !r.directiveOfferIds.includes(id))
        return false; const d = this.directives.find((x) => x.id === id); if (!d)
        return false; r.selectedDirective = id; r.directiveLocked = true; this.post(`DIRECTIVE LOCKED FOR THE NEXT CIVILIZATION: ${d.name}`); this.save(); this.emit(); return true; }
    selectBreedingMatrix(id) { const r = this.state.machine.runBuild; if (!this.systemUnlocked('breeding_matrices') || r.matrixLocked || !this.state.meta.progression.knownBreedingMatrices.includes(id))
        return false; const d = this.matrices.find((x) => x.id === id); if (!d)
        return false; r.selectedBreedingMatrix = id; r.matrixLocked = true; r.previewTraitIds = this.buildTraitSelection(r.nextCivilizationSeed).ids; this.post(`BREEDING MATRIX LOCKED FOR THIS UNIVERSE: ${d.name}`); this.save(); this.emit(); return true; }
    ...
    runtimeBonuses() {
        ...
        const containmentRating = ['reality_lattice', 'awareness_scrubber', 'sanity_protocol', 'cosmic_muffling'].reduce((sum, id) => sum + (l('machine', id) > 0 ? 1 : 0), 0);
        const temporalLevel = Math.max(0, Math.min(3, l('machine', 'temporal_injector')));
        const stableLevel = l('universe', 'stable_constants');
    ...
    startCivilization(requestedSeed = 0) {
        ...
        civ.harvestMult = { causal_mass: bonuses.causal_massMult, cognition: bonuses.cognitionMult, paradox: bonuses.paradoxMult, existence: bonuses.existenceMult };
    ...
    tick(delta) {
        ...
        decay += cascadeDecay(civ.tactical.entropy);
    ...
    previewEventChoiceEffects(choice) { const civ = this.state.civilization; if (!civ)
        return {}; const effects = structuredClone(CivilizationPaths.mergedChoiceEffects(civ, choice)); const b = this.runtimeBonuses(); for (const key of ['stability', 'awareness', 'sanity', 'attention']) {
        if (effects[key] == null)
            continue;
    ...
    resetMachineLayer() { this.state.machine.currencies = { causal_mass: 0, cognition: 0, paradox: 0, existence: 0 }; this.state.machine.upgradeLevels = {}; this.state.machine.activeMutations = []; this.state.machine.civilizationsThisUniverse = 0; this.state.machine.cultivationCreditsThisUniverse = 0; this.state.machine.lastHarvest = {}; this.state.machine.runBuild = { selectedDirective: '', selectedBreedingMatrix: '', directiveLocked: false, matrixLocked: false, directiveOfferIds: [], nextCivilizationSeed: 0, previewTraitIds: [] }; this.state.civilization = null; this.state.phase = 'machine'; this.state.simulationSpeed = 1; this.decisionFeedback = null; this.worldImpulse = null; this.prepareNextRun(0, false); }
    presentNextEvent(civ) { this.decisionFeedback = null; this.lastActionFailure = ''; civ.tactical.probedEventId = ''; const e = this.selectEvent(civ) ?? this.eventById('routine_compliance_audit'); if (!e)
        return; civ.pendingEvent = e.id; civ.eventTimer = 0; this.save(); this.emit(); }
    selectEvent(civ) {
        ...
        const stateMultiplier = (event) => { const s = civ.stats, id = event.id; let weight = 1; if (s.sanity < 50 && ['first_machine_cult', 'probability_strike', 'ministry_of_sanity', 'reality_unionizes'].includes(id))
            weight *= 1.8; if (s.attention > 50 && ['entity_audit', 'cosmic_predator', 'sky_inventory'].includes(id))
    ...
    eventEligible(e, civ) { if (civ.era < Number(e.min_era ?? 0) || civ.era > Number(e.max_era ?? 2))
        ...
        return false; if (r.min_awareness != null && s.awareness < Number(r.min_awareness))
        return false; if (r.max_awareness != null && s.awareness > Number(r.max_awareness))
        return false; if (r.max_sanity != null && s.sanity > Number(r.max_sanity))
        return false; if (r.min_sanity != null && s.sanity < Number(r.min_sanity))
        return false; if (r.max_stability != null && s.stability > Number(r.max_stability))
        return false; if (r.requires_trait && !civ.traits.includes(String(r.requires_trait)))
        return false; if (r.requires_flag && !civ.flags.includes(String(r.requires_flag)))
        return false; if (r.required_institution && !civ.institutions.includes(String(r.required_institution)))
        return false; if (r.excluded_flag && civ.flags.includes(String(r.excluded_flag)))
        return false; if (r.min_development != null && civ.development < Number(r.min_development))
        return false; return true; }
    applyEffects(civ, effects, resilience) {
        ...
        for (const [key, val] of Object.entries(effects)) {
            ...
            if (['stability', 'awareness', 'sanity', 'attention'].includes(key)) {
                ...
                civ.developmentMultiplier = Math.max(.2, civ.developmentMultiplier + Number(value));
            else if (key === 'event_delay')
                civ.eventDelayBonus += Number(value);
            else if (key === 'stability_decay_mult')
                civ.stabilityDecayMult = Math.max(.1, civ.stabilityDecayMult * Number(value));
    ...
    clampStats(civ) { const s = civ.stats; s.stability = Math.max(0, Math.min(s.stabilityMax, s.stability)); s.awareness = Math.max(0, Math.min(100, s.awareness)); s.sanity = Math.max(0, Math.min(100, s.sanity)); s.attention = Math.max(0, Math.min(100, s.attention)); }
    rollEventDelay(civ) { const rng = new SeededRng(civ.rngState); const window = eventDelayWindow(civ); const d = rng.range(window.min, window.max) + civ.eventDelayBonus; civ.rngState = rng.state; return Math.max(5, d); }
    buildTraitSelection(seed) { const rng = new SeededRng(seed); const bonuses = this.runtimeBonuses(); const allowed = this.traits.filter((t) => !t.impossible || this.upgradeLevel('axiom', 'axiom_impossible_birth') > 0).slice(); const count = Math.min(allowed.length, 2 + Math.trunc(bonuses.extraTraits)); const ids = []; for (let i = 0; i < count; i++) {
        const total = allowed.reduce((sum, trait) => sum + this.traitWeight(trait.id), 0);
    ...
    enterEra(civ, newEra) { civ.era = newEra; civ.tactical.controlCapacity = Math.min(3, civ.tactical.controlCapacity + 1); this.appendHistory(civ, `YEAR ${Math.trunc(civ.years)}: Civilization enters ${ERA_NAMES[newEra]}.`); this.post(`Civilization entered ${ERA_NAMES[newEra]}. Control Capacity +1.`); }
    appendHistory(civ, msg) { civ.history.unshift(msg); civ.history = civ.history.slice(0, 80); }
}
