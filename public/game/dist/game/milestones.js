...
export function evaluateMilestones(state, convergenceUnlocked) {
    ...
    for (const milestone of MILESTONE_CATALOG) {
        if (done[milestone.id])
            continue;
        if (!snapshot)
            snapshot = milestoneSnapshot(state, convergenceUnlocked);
        if (milestone.current(snapshot) < milestone.target)
            continue;
        done[milestone.id] = true;
        state.meta.progression.machineInsight += milestone.insight;
        insightAwarded += milestone.insight;
