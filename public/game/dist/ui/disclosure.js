...
export function isDisclosureOpen(id) { return openDisclosures.has(id); }
/** ` open` for a disclosure the player has opened, otherwise nothing. Renders into the tag. */
export function disclosureAttr(id) { return isDisclosureOpen(id) ? ' open' : ''; }
export function setDisclosureOpen(id, open) {
    if (open)
        openDisclosures.add(id);
    else
        openDisclosures.delete(id);
}
/**
 ...
 */
export function bindDisclosureListener(doc) {
    const flag = doc;
    if (flag.__disclosureListenerBound)
        return;
    ...
    doc.addEventListener('toggle', event => {
        ...
        if (id)
            setDisclosureOpen(id, target.open);
    }, true);
}
