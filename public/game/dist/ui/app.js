...
export function createGameUI(engine, world) {
    ...
    function renderCivilization(vm) {
        ...
        const reserveCard = vm.machineReserve.length ? card('Machine Reserve', `<p class="panel-note">Commit banked resources to the running civilization. Each use triples the price, and the price rises with the depth already reached.</p><div class="reserve-actions">${vm.machineReserve.map((entry) => `<div class="tactical-action-wrap"><button data-action="reserve" data-id="${esc(entry.id)}" aria-describedby="reserve-reason-${esc(entry.id)}" ${entry.enabled ? '' : 'disabled'}><span>${esc(entry.title)}</span><b>${esc(entry.summary)}</b><small>COST ${fmt(entry.cost)} ${esc(entry.currency.replaceAll('_', ' '))} · ${entry.usesLeft} OF ${entry.maxUses} LEFT</small></button><span id="reserve-reason-${esc(entry.id)}" class="tactical-reason">${esc(entry.reason)}</span></div>`).join('')}</div>`, 'machine-reserve') : '';
