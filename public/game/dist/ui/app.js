...
function revealTutorialAnchor(tutorialLayer, view) {
...
function renderMachine(machine, vm, engine) {
    const t = text().ui.app;
...
const rewardText = (key, details, engine) => `${resourceShort()[key]} ${key === 'causal_mass' || engine.resourceDiscovered(key) ? fmt(details.rewards[key]) : '???'}`;
const rewardSpan = (kind, key, details, engine) => `<span data-live="harvest-${kind}-${key}">${esc(rewardText(key, details, engine))}</span>`;
const urgencyText = (h) => {
    const t = text().ui.app;
    const u = h.urgency;
...
function renderWorldHudAndDock(worldHud, worldStatusDock, c, path, vm, t) {
...
function buildRecordDetails(vm, c, engine, harvest, chaotic, t) {
...
function renderCivilization(worldHud, worldStatusDock, civPanels, worldShell, vm, engine, stateCtx) {
    const c = vm.civilization;
    if (!c)
        return;
    const t = text().ui.app;
...
function refreshCivilizationLive(civPanels, worldHud, worldStatusDock, vm, engine) {
    ...
    for (const key of visibleKeys(vm, CONTROLLED_KEYS))
        setText(`[data-live="harvest-controlled-${key}"]`, rewardText(key, vm.harvest.controlled, engine));
    for (const key of visibleKeys(vm, CHAOTIC_KEYS))
        setText(`[data-live="harvest-chaotic-${key}"]`, rewardText(key, vm.harvest.chaotic, engine));
...
function renderVictory(victoryView, vm) { const record = vm.victory?.record; if (!record) {
...
function bindActions(engine, world) { document.querySelectorAll('[data-action]').forEach(el => { if (el.dataset.bound)
...
export function createGameUI(engine, world) {
    ...
    const render = () => {
        ...
        if (stepKey !== lastTutorialStepId) {
            ...
            revealTutorialAnchor(tutorialLayer, vm.tutorial);
        }
        if (explainToggle) {
