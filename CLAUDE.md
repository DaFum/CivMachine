...
npm test             # compiles public/game/src, then runs all test suites (289 tests)
...
### The Civilization Drama Arc (v1.10.0)

Semantics live in the game layer, drawing in the render layer, and the boundary is one-way:

- **`game/decision-consequences.ts`** classifies a completed decision from its before/after `DecisionSnapshot` into a significance, a tag list, and structural transitions. It never reads player-facing copy. **`game/consequence-profiles.ts`** holds exactly 28 typed signature profiles keyed by event id (plus, for institutions, a required addition).
- **`game/world-memory.ts`** reduces those consequences into an optional `Civilization.visualMemory`: at most 6 marks (one per domain) and 3 evolving scars, with deterministic FNV-1a anchors. It is **presentation-only** — no progression, pressure, harvest, or scheduler module may read it, and `tests/core.test.mjs` pins that harvest and depth are unchanged by setting it. `engine.publishCompletedDecision` is the single wiring point; the `tick()` Entropy-threshold impulse deliberately does not go through it.
- **`render/identity.ts`** derives a path identity tier (1 leading, 2 dominant, 3 consolidated/endgame) plus institution landmarks, and owns both the capital silhouette (`drawIdentityLandmarks`, cached scenery) and the ambient path motifs (`drawPathAmbience`, dynamic layer).
- **`render/world-memory.ts`** draws marks and scars on the cached scenery layer; only a scar halo animates. **`render/consequence-presentation.ts`** turns feedback into a transient dynamic-layer impact and the passive phase-transition cue.
- **`render/quality.ts`** is renderer-local adaptive quality: degrade after 30 frames over 24 ms, recover after 180 under 14 ms, 5 s cooldown. It sheds particles, haze and cosmetic agents only — never fractures, beacons, landmarks, scars or the current impact — and never touches `GameState` or `simulationSpeed`.
- `visualMemory` is the only new persistent field, and it is optional so v4 saves load without a `SAVE_VERSION` bump.
...
- Adding or renaming a module under `public/game/dist/` requires adding it to `APP_ASSETS`. `public/game/tests/presentation.test.mjs` asserts that every compiled `dist/**/*.js` module is listed, derived from disk rather than from a second hand-kept list.
...
The current version (`1.10.0`) appears in `package.json`, `public/game/package.json`, the footer of `public/game/index.html`, `CACHE_NAME` in `public/sw.js`, and the title plus a release-notes heading in both READMEs.
